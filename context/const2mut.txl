...
function main
    replace [program]
        P [program]
    construct AllConstItems [ConstantItem*]
        _ [^ P]
    construct AllConstIds [ConstId*]
        _ [^ AllConstItems]
    construct UpdatedConstIds [id*]
        _ [_onlyUpdatedConstIds P each AllConstIds]
    by
        P   [_const2mut each UpdatedConstIds]
end function

function _onlyUpdatedConstIds P [program] ConstId [ConstId]
    deconstruct ConstId
        Id [id]
    deconstruct * [ExpressionWithOrWithoutBlock] P
        Id _ [Infix_UpdateOperatorExpression] _ [Infix_Postfix_Expressions*]
    replace [id*]
        ids [id*]
    by
        Id
...
rule _const2mut Id [id]
    replace [VisibleItem]
        'const Id ': Type [Type] '= Expression [Expression] ';
    by
