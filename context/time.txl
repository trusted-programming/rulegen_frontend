...
% transform C time(time_t*) function call to Rust SystemTime::now()
function main
    replace [program]
        P [program]
    deconstruct * [ExternalFunctionItem] P
        fn time ( _ [id]: _ [TypeNoBounds] ) -> ReturnType [TypeNoBounds];
    construct UseSystemTime [Item?]
        _ [_getUseSystemTime]
    construct FuncRustTime [Item?]
        _ [_getFuncTime ReturnType]
    by
        P   [prependItemInProgram2 FuncRustTime ""]
            [prependItemInProgram2 UseSystemTime ""]
            [convertTime]
            [convertTimeZero]
            [cleanUpTime ReturnType]
end function

function _getUseSystemTime
    replace [Item?]
        _ [Item?]
    construct UseSystemTime [Item]
...
function _getFuncTime ReturnType [TypeNoBounds]
    replace [Item?]
        _ [Item?]
    construct FuncRustTime [Item]
        pub fn rust_time(ref_result: Option<&mut ReturnType>) -> ReturnType {
...
rule convertTime
    replace [Expression]
        time ( CallParam [CallParams?] ) Infix_Postfix_Expressions [Infix_Postfix_Expressions*]
    deconstruct CallParam
        NULL _ [Infix_Postfix_Expressions*]
    by
        rust_time(None)
        Infix_Postfix_Expressions [_castLitChar]
end rule

rule convertTimeZero
    replace [Expression]
        time ( CallParam [CallParams?] ) Infix_Postfix_Expressions [Infix_Postfix_Expressions*]
    deconstruct CallParam
        '0 _ [Infix_Postfix_Expressions*]
    by
        rust_time(None)
        Infix_Postfix_Expressions [_castLitChar]
end rule

function cleanUpTime ReturnType [TypeNoBounds]
    replace [program]
        utf8bom [UTF8BOM_NL?]
        shebang [SHEBANG_NL?]
    ...
    by
        ...
        shebang
        inner_attrs
        items   [cleanExtFuncDecl "time"]
                [rmItemN NRustTime UseSystemTime ""]
                [rmItemN NRustTime FuncRustTime ""]
end function
