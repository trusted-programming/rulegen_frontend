...
function main
    ...
    % there is a call to `atoi`
    deconstruct * [Expression] P
        atoi _ [Postfix_CallExpression] _ [Infix_Postfix_Expressions*]

    % there is no `atoi` declared as external function
    deconstruct not * [ExternalFunctionItem] P
        fn atoi _ [Generics?] ( _ [NamedFunctionParameters_or_NamedFunctionParametersWithVariadics?] )
            _ [FunctionReturnType?] _ [WhereClause?]
            _ [SEMI_or_BlockExpression]

    % there is no `atoi` declared as function (usually with `extern "C"` qualifiers)
    deconstruct not * [Function] P
        _ [FunctionQualifiers] 'fn atoi _ [Generics?] '( _ [FunctionParameters?] ')
            _ [FunctionReturnType?] _ [WhereClause?]
            _ [SEMI_or_BlockExpression]

    % there is no `use libc :: atoi;`
    construct UseLibcAtoI [Item]
        use libc :: atoi;
    deconstruct not * [Item] P
        UseLibcAtoI
    construct Item [Item?]
        UseLibcAtoI
