...
function main
    replace [program]
        P [program]
    construct AllConstI32Zeros [PubConstI32Zero*]
        _ [^ P] [print]
    construct Length [number]
        _ [length AllConstI32Zeros]
    where
        Length [> 0]
    construct PubConstNull [Item]
        'pub 'const NULL ': i32 '= 0 ';
    by
        P   [_rmConstI32Zero]
            [prependItemInProgram PubConstNull]
            [_replaceId 'NULL each AllConstI32Zeros]
end function

rule _rmConstI32Zero
    replace [Item*]
        Head [PubConstI32Zero]
        Tail [Item*]
    by
        Tail
end rule

rule _replaceId Id [id] PubConstI32Zero [PubConstI32Zero]
    deconstruct PubConstI32Zero
        'pub 'const Id2Rm [id] ': i32 '= 0 ';
    deconstruct not Id2Rm
        Id
    replace [id]
        Id2Rm
    by
